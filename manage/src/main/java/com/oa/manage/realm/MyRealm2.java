...
public class MyRealm2 extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        TUser tUser = empService.findTUser(uid);
        System.out.println("tUser: "+tUser);
        if(tUser != null){
            //有这个admin对象--->核对密码
            SimpleAuthenticationInfo info = new SimpleAuthenticationInfo(tUser,tUser.getPassword(), ByteSource.Util.bytes("hello"),"myRealm");//"myRealm"指定自定义的Realm名字,可以随便写
            return info;
        }
